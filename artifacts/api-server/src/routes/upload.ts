...
function mapRowToItem(row: Record<string, unknown>): {
  ...
  const produto = String(
    normalized["produto"] ?? normalized["descricao"] ?? normalized["sku"] ?? normalized["item"] ?? ""
  ).trim();

  if (!produto) return null;

  const marca = String(normalized["marca"] ?? normalized["fornecedor"] ?? normalized["brand"] ?? "").trim();
  const embalagem = String(normalized["embalagem"] ?? normalized["unidade"] ?? normalized["emb"] ?? "").trim();
  const doi = parseFloat(String(normalized["doi"] ?? normalized["diasestoque"] ?? normalized["diainventario"] ?? "0")) || 0;
  const rawStatus = String(normalized["status"] ?? normalized["situacao"] ?? "OK").trim().toUpperCase();
  const status = rawStatus === "NOK" ? "NOK" : "OK";
  const demanda = parseFloat(String(normalized["demanda"] ?? normalized["venda"] ?? normalized["consumo"] ?? "0")) || 0;
  const min = parseFloat(String(normalized["min"] ?? normalized["minimo"] ?? normalized["estoquemin"] ?? "0")) || 0;
  const max = parseFloat(String(normalized["max"] ?? normalized["maximo"] ?? normalized["estoquemax"] ?? "0")) || 0;
  const rawCurva = String(normalized["curva"] ?? normalized["curvaabc"] ?? normalized["abc"] ?? "C").trim().toUpperCase();
...
router.post("/estoque/upload", async (req, res): Promise<void> => {
  ...
  if (!fileName || !fileBase64) {
    res.status(400).json({ error: "fileName e fileBase64 são obrigatórios" });
    return;
  }
  ...
  try {
    ...
    res.status(400).json({ error: "Arquivo inválido. Use .xlsx ou .csv" });
    return;
  }
  ...
  const items = rows.map(mapRowToItem).filter((item): item is NonNullable<typeof item> => item !== null);

  if (items.length === 0) {
    res.status(400).json({ error: "Nenhum item válido encontrado na planilha" });
    return;
  }
