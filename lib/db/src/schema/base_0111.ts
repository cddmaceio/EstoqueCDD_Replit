...
export const base0111SnapshotsTable = pgTable(
  "base_0111_snapshots",
  {
  ...
  (table) => ({
    uploadedAtIdx: index("base_0111_snapshots_uploaded_at_idx").on(table.uploadedAt),
  }),
);

export const base0111Table = pgTable(
  "base_0111",
  {
    id: serial("id").primaryKey(),
  ...
  (table) => ({
    snapshotIdx: index("base_0111_snapshot_id_idx").on(table.snapshotId),
    codigoIdx: index("base_0111_codigo_idx").on(table.codigo),
    codigoSapIdx: index("base_0111_codigo_produto_sap_idx").on(table.codigoProdutoSap),
  }),
);
